...
def preprocess(params):
    pid,  nod_mask_dir, img_dir, save_dir, do_resample, savename = params

    print('Preprocessing %s...' % (pid))

    #lung_mask, _, _ = load_itk_image('%s.mhd' % (pid_mask))
    img, origin, spacing = load_itk_image('%s' % (pid))
    binary_mask1, binary_mask2 = extract_lung(img, origin)
...
def main():
    ...
    for line in record_name:
        line = line.rstrip()
        savename = line
        params_lists.append([os.path.join(mhd_dir, line+'.mhd'), nod_mask_dir, img_dir, save_dir, do_resample, savename])
    pool = Pool(processes=10)
